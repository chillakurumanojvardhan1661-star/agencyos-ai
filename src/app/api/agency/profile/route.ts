  ...
  try {
    ...
    if (existingAgency) {
      ...
      const { data, error } = await (supabase.from('agencies') as any)
        .update(input)
        .eq('id', (existingAgency as any).id)
        .select()
        .single();
